# Developer Guide: Sovereign Telemetry Reports
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/telemetry-reports.git
...
```bash
export SBB_REPORT_TEMPLATE_DIR=your_value_here
export SBB_OUTPUT_FORMAT=your_value_here
export SBB_SMTP_HOST=your_value_here
export SBB_CHART_THEME=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-telemetry-reports)
...
```bash
# Build
docker build -t sovereign-telemetry-reports:dev .

# Run with env file
docker run --env-file .env -p 8793:8793 sovereign-telemetry-reports:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-telemetry-reports
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8793`
...
requests.post(N8N_EVENT_BUS_URL, json={
    "source": "sovereign-telemetry-reports",
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8793` in use | `lsof -i :8793` | Kill PID or change `SERVICE_PORT` |
